...
const controller = {
  ...
  store: (req, res) => {
    ...
    products.push({
      ...
      image: req.file ? req.file.filename : "default-image.png", // como aún no sabemos subir archivos, dejamos para todos los productos nuevos esta imagen default
    });
  ...
  update: (req, res) => {
    ...
    const productsArrayEdited = products.map((oneProduct) => {
      if (oneProduct.id === id) {
        // si los ID's coinciden, editamos ese producto (objeto)
        return {
  ...
  destroy: (req, res) => {
    ...
    const productsArrayFiltered = products.filter(
    ...
    fs.writeFileSync(
      productsFilePath,
      JSON.stringify(productsArrayFiltered, null, " ")
    );
